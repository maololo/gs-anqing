				...
				success : function(data) {
					...
					for (var i = 0; i < data.length; i++) {
						...
						if (menuData.id != 24 && menuData.text != "系统管理") {
							...
							left_nav1_menu = left_nav1_menu
									+ '<li><a href="javascript:;" class="inactive"><span class="iconfont '
									+ imgSrc
									+ '" style="margin-right: 5px;"></span>'
									+ menuData.text
									+ '</a><ul style="display: none">';
							left_nav2_menu = left_nav2_menu
									+ '<li><a href="javascript:"><span class="iconfont '
									+ imgSrc
									+ '"></span></a><ul class="foldsecond"><li><a href="javascript:">'
									+ menuData.text
									+ '<span class="iconfont icon-xiala" style="float:right"></span></a><ul class="foldthird" style="display: none">';
							if (childrenData != undefined) {
								for (var j = 0; j < childrenData.length; j++) {
									var icon = childrenData[j].attributes.C_ICONCLS;
									left_nav1_menu = left_nav1_menu
											+ "<li ><a href='javascript:;' onclick=res('"
											+ childrenData[j].attributes.C_URL
											+ "','"
											+ childrenData[j].attributes.C_ISOUTLINK
											+ "','"
											+ childrenData[j].attributes.C_ISCOMMON
											+ "',this);>"
											+ childrenData[j].text
											+ "</a></li>";
									left_nav2_menu = left_nav2_menu
	...
	if (id == "") {
		...
		if (field != "" && value == "") {
			...
			var timingFunction = setInterval(function() {
				...
				vector_Source.forEachFeatureInExtent(boxExtent, function(
						feature) {
...
function res(url, outlink, common, self, callback) {
	...
	if (self != undefined && self != '') {
		...
		$('#center-title').html(
				childNodes[0].innerText + " / " + self.innerHTML);
	}
	if (outlink == 1) {
